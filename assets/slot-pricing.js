...
export function mountSlotCalculator(root, config, lang = 'en', initialSlots = 3) {
 ...
 const calculate = () => {
  ...
  try {
   ...
   out.innerHTML = `<p><strong>${estimate.level[0].toUpperCase()+estimate.level.slice(1)}</strong> · ${estimate.credits.toLocaleString()} ${w.credits}/${w.month}</p>` +
...
export function mountSlotPricing(root, config, lang = 'en') {
 ...
 const w=pricingWords(lang);
 root.innerHTML=`<div class="section-inner slot-pricing"><h2>${w.title}</h2><div class="slot-offer">
 <section><h3>${w.free}</h3><strong>0 NOK</strong><p>${w.freeNote}</p><a href="https://clientrelay.tech/app/?lang=${encodeURIComponent(lang)}">${w.continue}</a></section>
 ...
 <output aria-live="polite"></output><p data-slot-credits></p><p>${slotIncludedText(config,lang)}</p><a href="https://clientrelay.tech/app/?lang=${encodeURIComponent(lang)}">${w.continue}</a></section></div>
 <p>${w.pool}</p>${levelsHtml(config,lang)}</div>`;
 const update=()=>{try {const e=estimateSlots(config,Number(root.querySelector('[name=pricing-slots]').value)); root.querySelector('output').textContent=`${nok(e.amount,lang)} / ${w.month} · ${w.exVat}`; root.querySelector('[data-slot-credits]').textContent=`${e.credits.toLocaleString()} ${w.credits}/${w.month} · ${e.level[0].toUpperCase()+e.level.slice(1)}`;} catch {root.querySelector('output').textContent='—';root.querySelector('[data-slot-credits]').textContent='';}};
